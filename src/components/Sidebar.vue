        ...
        <div class="row justify-content-end h-100">
            ...
            <transition name="slide">
                <div class="col-md-3 col-sm-12 hidden-md-down animate__animated animate__slideInRight" id="yellow">
                    <div class="row">
                        <div class="col-2">
                            <button class="btn btn-light rounded-circle border float-left shadow-sm mt-1" @click="closeSidebarPanel">&lt;</button>
                        </div>
                        <div class="col">
                            <h6 class="text-center mt-3">YOUR CART ({{ cartItems.length }})</h6>
                        </div>
                        <div class="col-2 mb-3">
                            <select class="form-control w-auto form-control-s mt-2 form-control-sm float-right" v-model="cartCurrency" @change="getProducts">
                                <option v-for="(item, index) in currency" :key="index">{{ item }}</option>
                            </select>
                        </div>
                    </div>

                    <div v-if="cartItems.length > 0" class="cart-container p-0">
                    ...
                    <div v-if="cartItems.length > 0" class="cart-footer" style="background-color: #feff00;">
                        <hr class="m-1" />
                    ...
                    <div class="mt-5 pt-5" v-if="cartItems.length < 1">
